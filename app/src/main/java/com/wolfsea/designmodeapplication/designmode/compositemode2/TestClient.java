...
public class TestClient {
    ...
    public static void main(String[] args) {
        ...
        IBranch root = new Branch("李大大", "总经理", 20000);
    ...
    public static void getSubOrdinateInfo(ArrayList<ICrop> subOrdinateInfo) {

        final int SIZE = subOrdinateInfo.size();
        for (int i = 0; i < SIZE; i++) {

            final ICrop crop = subOrdinateInfo.get(i);
            if (crop instanceof ILeaf) {

                ILeaf leaf = (ILeaf) crop;
                System.out.println(leaf.getInfo());
            } else {

                IBranch branch = (IBranch) crop;
